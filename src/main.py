...
############################################################
    ...
    for ind, zplan in enumerate(zp_ind):
        ...
        base_path = os.path.join(os.path.dirname(__file__), "../")
        _, _, _, _, _, _, _, _, ko2_tke, ko2_omega = read_rans(os.path.join(base_path,rans_path))
        ...
        # kx = np.logspace(int(kstart),int(kend)+1, ko2_tke.shape[0]-1)
        # kx = np.append(0,kx)
        # kx = read_moser('input/Moser/Re395/Spectra/Streamwise/chan395.xspec.10.txt')[0]
        
        print('Le shape:', Le.shape)
        print('kx shape:', ko2_omega.shape)
        
        phi11 = phi_11(ko2_omega, 1.0, sigma_u, Le)
        phi22 = phi_22(ko2_omega, 1.0, sigma_v, Le)
        phi33 = phi_22(ko2_omega, 1.0, sigma_w, Le)
        ...
        int_theo1 = np.trapz(phi11, ko2_omega)
        int_theo2 = np.trapz(phi22, ko2_omega)
        int_theo3 = np.trapz(phi33, ko2_omega)
        ...
        von_karman_plot(fig_vanK, col, row, ko2_omega, phi11, name = '$\phi^{1}_{11}\\text{VK}$', color = 'firebrick', symbols='circle-open')
        von_karman_plot(fig_vanK, col, row, ko2_omega, phi22, name = '$\phi^{1}_{22}\\text{VK}$', color = 'midnightblue', symbols='circle-open')
        von_karman_plot(fig_vanK, col, row, ko2_omega, phi33, name = '$\phi^{1}_{33}\\text{VK}$', color = 'darkgreen', symbols='circle-open')
        
        if chplot == 'all':
            if split_time == 'Y':
                save_datas([ko2_omega, phi11, phi22, phi33], ['kx', 'phi11', 'phi22', 'phi33'], f'split_time/von_karman/vk_spactra_{zp[ind]}.dat', 'von Karman spectra')
            if split_time == 'n':
                save_datas([ko2_omega, phi11, phi22, phi33], ['kx', 'phi11', 'phi22', 'phi33'], f'whole_time/von_karman/vk_spactra_{zp[ind]}.dat', 'von Karman spectra')
