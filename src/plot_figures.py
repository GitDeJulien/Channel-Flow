...
def init_figures_ft(z, ch='normal'):
    ...
    if ch == "all":
        ...
        fig2 = make_subplots(rows=2, cols=5, shared_yaxes='rows', vertical_spacing=0.15, subplot_titles=(f"$z^+={z[0]}$", f"$z^+={z[1]}$", f"$z^+={z[2]}$", f"$z^+={z[3]}$", f"$z^+={z[4]}$", f"$z^+={z[5]}$", f"$z^+={z[6]}$", f"$z^+={z[7]}$", f"$z^+={z[8]}$", f"$z^+={z[9]}$"))
...
def init_figures_gamma(z, ch='normal'):
    ...
    if ch == 'normal':
        figu1 = make_subplots(rows=1, cols=4, shared_yaxes=True, y_title='$log(\phi(r_1,\omega)e^{-ik_cr_1} / \phi_{UU}(\omega))$', subplot_titles=(f"$z^+={z[0]}$", f"$z^+={z[1]}$", f"$z^+={z[2]}$", f"$z^+={z[3]}$"))
        figu2 = make_subplots(rows=1, cols=4, shared_yaxes=True, y_title='$log(\phi(r_1,\omega)e^{-ik_cr_1} / \phi_{VV}(\omega))$', subplot_titles=(f"$z^+={z[0]}$", f"$z^+={z[1]}$", f"$z^+={z[2]}$", f"$z^+={z[3]}$"))
...
def init_figures_vk(z, ch='normal'):
    ...
    if ch == 'normal':
        fig = make_subplots(rows=1, cols=4, shared_yaxes=True, subplot_titles=(f"$z^+={z[0]}$", f"$z^+={z[1]}$", f"$z^+={z[2]}$", f"$z^+={z[3]}$"))
...
def von_karman_plot(fig, col, row, kc, phi, name = 'corr', color = 'firebrick', symbols='circle', line='no'):
    
    if line == 'no':
        if col == 4 and row == 1:
            fig.add_trace(go.Scatter(x=kc, y=phi, name=name, mode= 'lines+markers', line=dict(color=color, width=1), marker=dict(color=color, symbol=symbols)), row=row, col=col)
            ...
            fig.add_trace(go.Scatter(x=kc, y=phi, name=name, mode= 'lines+markers', line=dict(color=color, width=1), marker=dict(color=color, symbol=symbols), showlegend=False), row=row, col=col)
            lin1 = np.logspace(0,5)
            fig.add_trace(go.Scatter(x=lin1, y=lin1**(-5./3), line=dict(color='darkmagenta', dash='dash', width=2), showlegend=False), row=row, col=col)
            fig.update_xaxes(title='$k_x$', type="log", exponentformat='power', row=row, col=col, range=[0,5])
            fig.update_yaxes(type="log", exponentformat='power', row=row, col=col, range=[-10,0])
        ...
        fig.add_annotation(x=2, y=-2, text="$k_x^{-5/3}$", showarrow=False, row=row, col=col)
    
    else:
        if col == 4 and row == 1:
            fig.add_trace(go.Scatter(x=kc, y=phi, name=name, mode= 'lines', line=dict(color=color, width=2)), row=row, col=col)
            lin1 = np.logspace(0,5)
            fig.add_trace(go.Scatter(x=lin1, y=lin1**(-5./3), line=dict(color='darkmagenta', dash='dash', width=2), showlegend=False), row=row, col=col)
            fig.update_xaxes(title='$k_x$', type="log", exponentformat='power', row=row, col=col, range=[0,5])
            fig.update_yaxes(type="log", exponentformat='power', row=row, col=col, range=[-10,0])
            ...
            fig.add_trace(go.Scatter(x=kc, y=phi, name=name, mode= 'lines', line=dict(color=color, width=2), showlegend=False), row=row, col=col)
            lin1 = np.logspace(0,5)
            fig.add_trace(go.Scatter(x=lin1, y=lin1**(-5./3), line=dict(color='darkmagenta', dash='dash', width=2), showlegend=False), row=row, col=col)
            fig.update_xaxes(title='$k_x$', type="log", exponentformat='power', row=row, col=col, range=[0,5])
            fig.update_yaxes(type="log", exponentformat='power', row=row, col=col, range=[-10,0])
...
def space_correlation_plot(fig, col, row, Dx, corr, name = 'corr', color = 'firebrick', axis = 'streamwise'):
    ...
    if col == 1:
        if axis == 'streamwise':
            fig.update_yaxes(row=row, col=col, title_text = "$R_{ii}(\delta x)$")
        if axis == 'spanwise':
            fig.update_yaxes(row=row, col=col, title_text = "$R_{ii}(\delta y)$")
...
def gamma_plot(fig, col, row, funct, omega, Dx, ind0, ind1, ch = 'w'):
    ...
    if ch == 'w':
        if col == 1 and row == 1:
            ...
            fig.add_trace(go.Scatter(x=omega[ind0:ind1], y=funct[ind0:ind1,10], mode='lines', name=f'$r={Dx[10]:.2f}$', line=dict(color='purple')), row=row, col=col)
            fig.add_trace(go.Scatter(x=omega[ind0:ind1], y=funct[ind0:ind1,15], mode='lines', name=f'$r={Dx[15]:.2f}$', line=dict(color='mediumvioletred')), row=row, col=col)
            ...
            fig.add_trace(go.Scatter(x=omega[ind0:ind1], y=funct[ind0:ind1,10], mode='lines', showlegend=False, line=dict(color='purple')), row=row, col=col)
            fig.add_trace(go.Scatter(x=omega[ind0:ind1], y=funct[ind0:ind1,15], mode='lines', showlegend=False, line=dict(color='mediumvioletred')), row=row, col=col)
    ...
    if ch == 'x':
        if col == 4 and row == 1:
            ind1
            moy1 = np.mean(funct[1, ind0:ind1])
            moy2 = np.mean(funct[2, ind0:ind1])
            ...
            fig.add_trace(go.Scatter(x=Dx[ind0:ind1]-xlen//2, y=funct[3,ind0:ind1], mode='lines', name=f'$\omega={omega[3]:.2f}$', line=dict(color='purple')), row=row, col=col)
            fig.add_trace(go.Scatter(x=Dx[ind0:ind1]-xlen//2, y=funct[4,ind0:ind1], mode='lines', name=f'$\omega={omega[4]:.2f}$', line=dict(color='mediumvioletred')), row=row, col=col)
